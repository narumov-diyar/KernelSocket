 ...
 * Реализует паттерн безопасной выгрузки модуля через `kernel_sock_shutdown` для
 * разблокировки ожидающих сокетов.
 */
...
#include "../../src/linux/ks_linux_internal.h" 
...
/** @brief Локальный TCP-порт для прослушивания сервером. */
#define TEST_PORT_TCP 9000

/** @brief Локальный UDP-порт для прослушивания сервером. */
#define TEST_PORT_UDP 9001

/** @brief IP-адрес для подключения клиентов. */
#define TARGET_IP "x.x.x.x" /* Замените на IP напарника (Windows) для сетевого теста */

/* ========================================================================= */

/** @brief Глобальный указатель на сокет TCP-сервера (нужен для экстренного прерывания). */
static KS_SOCKET *G_TcpServerSock = NULL;

/** @brief Глобальный указатель на сокет UDP-сервера (нужен для экстренного прерывания). */
static KS_SOCKET *G_UdpServerSock = NULL;

/** @brief Глобальные указатели на системные потоки (kthreads). */
...
/**
 * @brief Формирует текущее время в виде строки `ЧЧ:ММ:СС`.
 *
 * @param[out] buffer Буфер для записи отформатированного времени.
 * @param[in]  size   Размер переданного буфера.
 */
...
/**
 * @brief Унифицированный вывод логов тестового стенда Linux.
 *
 * @details Форматирует сообщение и выводит его в кольцевой буфер ядра (`dmesg`).
 *
 * @param[in] tag Тег подсистемы/потока (например, "TCP-СЕРВЕР").
 * @param[in] fmt Строка формата (`printf`-совместимая).
 * @param[in] ... Дополнительные аргументы форматирования.
 */
...
/**
 * @brief Поток TCP-сервера тестового стенда.
 * 
 * @details Слушает порт `TEST_PORT_TCP`. Принимает соединения, читает данные 
 * и отправляет клиенту квитанцию с подтверждением.
 *
 * @param[in] arg Не используется (ожидается `NULL`).
 * 
 * @return `0` при штатном завершении, отрицательный код при ошибке запуска.
 */
...
/**
 * @brief Поток TCP-клиента тестового стенда.
 * 
 * @details Генерирует и отправляет на `TARGET_IP` серию пакетов. 
 * После каждой отправки ожидает эхо/квитанцию.
 *
 * @param[in] arg Не используется (ожидается `NULL`).
 * 
...
/**
 * @brief Поток UDP-сервера тестового стенда.
 * 
 * @details Слушает порт `TEST_PORT_UDP` и логирует все входящие датаграммы.
 *
 * @param[in] arg Не используется (ожидается `NULL`).
 * 
 * @return `0` при штатном завершении, отрицательный код при ошибке запуска.
...
/**
 * @brief Поток UDP-клиента тестового стенда.
 * 
 * @details Осуществляет периодическую отправку случайного количества 
 * датаграмм на адрес `TARGET_IP`.
 *
 * @param[in] arg Не используется (ожидается `NULL`).
 * 
 * @return `0` при штатном завершении.
...
/**
 * @brief Точка входа Linux-модуля KernelSocket.
 * 
 * @details Инициализирует тестовый стенд. В отличие от Windows, в Linux 
 * не требуется явная инициализация сетевого стека, поэтому функция сразу 
 * запускает фоновые `kthread` потоки.
 *
 * @return `0` при успешной инициализации тестовых потоков.
 */
...
/**
 * @brief Точка выгрузки Linux-модуля KernelSocket.
 *
 * @details
 * Выполняет мягкую остановку потоков и принудительно разблокирует
 * блокирующие сетевые вызовы (`ksAccept`, `ksRecvFrom`) серверов 
 * через вызов `kernel_sock_shutdown`.
 */
