/**
 * @file driver_entry.c
 * @brief Точка входа драйвера Windows и встроенный тестовый стенд.
 *
 * @details
 ...
 * параллельного тестирования протоколов. Поддерживает безопасную мягкую выгрузку
 * через архитектурный паттерн "Loopback Wake-up".
 */
...
/** @brief Глобальный флаг для безопасной остановки блокирующих потоков при выгрузке драйвера. */
 ...
 *
 * @param[in] Tag    Строковый тег подсистемы (например, "TCP-СЕРВЕР").
 * @param[in] Format Строка формата (printf-style).
 * @param[in] ...    Дополнительные аргументы для форматирования.
 */
 ...
 *
 * @param[out] Buffer     Указатель на буфер для записи (формат: ЧЧ:ММ:СС).
 * @param[in]  BufferSize Размер предоставленного буфера.
 */
 ...
 * 
 * @details Демонстрирует надежный обмен. Принимает подключение, читает данные, 
 * отправляет клиенту квитанцию (ACK) и корректно закрывает соединение.
 *
 * @param[in] Context Пользовательский контекст (ожидается NULL, не используется).
 */
 ...
 * 
 * @details Демонстрирует односторонний прием (Fire-and-Forget). Просто логирует 
 * полученные датаграммы без отправки ответа.
 *
 * @param[in] Context Пользовательский контекст (ожидается NULL, не используется).
 */
 ...
 * 
 * @details Демонстрирует UDP-стриминг. Отправляет серию датаграмм на целевой
 * адрес без ожидания квитанций.
 *
 * @param[in] Context Пользовательский контекст (ожидается NULL, не используется).
 */
...
/**
 * @brief Функция выгрузки драйвера (срабатывает при команде 'sc stop').
 * 
 * @details Устанавливает флаг `g_IsUnloading` и реализует безопасное завершение 
 * ожидающих потоков через отправку фиктивных сетевых пакетов на `127.0.0.1` 
 * (паттерн "Loopback Wake-up"), чтобы прервать блокирующие вызовы.
 *
 * @param[in] DriverObject Указатель на объект драйвера.
 */
...
/**
 * @brief Точка входа драйвера (срабатывает при загрузке 'sc start').
 * 
 * @details Инициализирует подсистему WSK и запускает системные потоки (kthreads) 
 * для тестирования API.
 *
 * @param[in] DriverObject Указатель на объект драйвера.
 * @param[in] RegistryPath Путь в реестре (не используется).
 * 
 * @return `STATUS_SUCCESS` при успехе, иначе код ошибки инициализации WSK.
 */
